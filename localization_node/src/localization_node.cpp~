...
localization_node::localization_node()
{
    ...
    odom_sub = n_.subscribe("/odom",100,&localization_node::odomCallback,this);
...
void localization_node::odomCallback(const nav_msgs::Odometry::ConstPtr &msg)
{
    ...
    pose_.x = pose_.x + currx;
    pose_.y = pose_.y + curry;
    ...
    if(callback_counter>20&&callback_counter%20==0)
    {
        //      calculate_weights();
        //     normalize_weights();
...
//        std::cout<<"Resampled "<<callback_counter/25<< "\n\n\n";
...
void localization_node::update_particles()
{
    ...
    std::normal_distribution<> motion_gauss(0.005);
    ...
    for(int i=0;i<N_PARTICLES-1;i++)
    {
        ...
        if(all_particles_[i].x==0&&all_particles_[i].y==0&&all_particles_[i].theta==0)
        {
            std::cout<<"Particle is 0 in update particles"<<std::endl;
        }
        /*
        all_particles_[i].theta = fmod(all_particles_[i].theta + rot_vel,6.28);
        all_particles_[i].x = all_particles_[i].x + v_vel*cos(all_particles_[i].x);
        all_particles_[i].y = all_particles_[i].y + v_vel*sin(all_particles_[i].y);
*/

        all_particles_[i].theta += rot_vel - ROT_SPREAD*rot_vel/2.0 + ROT_SPREAD*rot_vel*((double)std::rand() / (double)RAND_MAX);
        double rand_vel = ((double)std::rand() / (double)RAND_MAX);
        all_particles_[i].x += v_vel*cos(all_particles_[i].theta) - VEL_SPREAD*v_vel*cos(all_particles_[i].theta)/2.0 + v_vel*VEL_SPREAD*rand_vel*cos(all_particles_[i].theta);
        all_particles_[i].y += v_vel*sin(all_particles_[i].theta) - VEL_SPREAD*v_vel*sin(all_particles_[i].theta)/2.0 + v_vel*VEL_SPREAD*rand_vel*sin(all_particles_[i].theta);
...
{
    std::cout<<"Encoder callback called"<<std::endl;
    //Delta Calculations
